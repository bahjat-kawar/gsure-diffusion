...
def parse(args):
    ...
    if args.batch is not None:
        opt['datasets']["train"]['dataloader']['args']['batch_size'] = args.batch
    ...
    if args.debug:
        opt['gpu_ids'] = [0]
        opt['datasets']['train']['dataloader']['args']['batch_size'] = min(opt['datasets']['train']['dataloader']['args']['batch_size'], 2)
        opt['datasets']['train']['dataloader']['args']['num_workers'] = 0
        if 'debug' in opt:
            opt['train'].update(opt['debug'])

    ''' code backup '''
    for name in os.listdir('.'):
        if name in ['config', 'models', 'core', 'slurm', 'data', 'diffusion']:
