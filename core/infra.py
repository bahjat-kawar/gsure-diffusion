...
class Infra():
    def __init__(self, opt, phase_loader, val_loader, metrics, logger, writer, wandb_run, **kwargs):
    ...
    def print_network(self, network):
        ...
        if self.opt['global_rank'] !=0:
            return
        if isinstance(network, FFT_Wrapper):
    ...
    def save_network(self, network, network_label):
        ...
        if isinstance(network, nn.DataParallel) or isinstance(network, nn.parallel.DistributedDataParallel):
            network = network.module
        if isinstance(network, FFT_NN_Wrapper):
            network = network.model
    ...
    def load_network(self, network, network_label, strict=True):
        ...
        if not os.path.exists(model_path):
            ...
            print('Pretrained model in [{:s}] does not exist, Skip it'.format(model_path))
            return
